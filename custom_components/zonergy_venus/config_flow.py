...
async def async_validate_input(
    data: dict[str, Any],
    required_entity_names: set[str] | frozenset[str] = REQUIRED_ENTITY_NAMES,
...
class ZonergyVenusConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        return self.async_show_menu(
            step_id="user",
            menu_options=[CONNECTION_ESPHOME, CONNECTION_BLUETOOTH, CONNECTION_CLOUD],
        )
    ...
    async def async_step_bluetooth(
        ...
        """Configure the local ESPHome Bluetooth gateway."""
        errors: dict[str, str] = {}

        if user_input is not None:
            try:
                device_info, _ = await async_validate_input(
                    user_input, BLE_REQUIRED_ENTITY_NAMES
                )
            except (InvalidAuthAPIError, InvalidEncryptionKeyAPIError):
                errors["base"] = "invalid_auth"
            except CannotConnect:
                ...
                errors["base"] = "invalid_bluetooth_gateway"
            else:
                unique_id = device_info.mac_address.replace(":", "").lower()
                await self.async_set_unique_id(unique_id)
                self._abort_if_unique_id_configured(
                ...
                clean_data = dict(user_input)
                clean_data[CONF_CONNECTION_TYPE] = CONNECTION_BLUETOOTH
                clean_data[CONF_PASSWORD] = clean_data.get(CONF_PASSWORD, "")
                clean_data[CONF_ENCRYPTION_KEY] = clean_data.get(
                    CONF_ENCRYPTION_KEY, ""
                )
                title = device_info.friendly_name or device_info.name
                return self.async_create_entry(
                    title=title or "Zonergy Venus Bluetooth", data=clean_data
                )
        ...
        return self.async_show_form(
            step_id="bluetooth", data_schema=schema, errors=errors
        )
